    ...
    <div className={`relative min-h-screen ${isDarkTheme ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'} p-4 flex flex-col`}>
      <div className="space-y-2 relative z-10 mb-5 text-left">
        <div className="flex justify-between items-center">
          ...
          <button onClick={handleAuthencation} className="text-sm text-blue-500">Авторизация</button>
        </div>
        <h1 className="text-3xl font-bold text-blue-600">Регистрация</h1>
        <p className="text-2xl">в 15TH App!</p>
      </div>

      <div className="space-y-4 relative z-10 mb-5">
        <div>
          <label className="block text-gray-500">Ваше имя</label>
          <input
            type="email"
      ...
      <div className="flex-grow flex flex-col justify-center items-center relative">
        ...
        <div className="fixed right-0 w-1/2 flex justify-end">
          ...
          <button onClick={handleAuthencation} className="text-white absolute mt-10 font-bold py-2 px-6 rounded-lg transition-all text-center text-lg md:text-xl lg:text-2xl">
            Создать Аккаунт
          </button>
        </div>
      </div>
